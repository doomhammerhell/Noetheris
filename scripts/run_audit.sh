...
"${NOETHERIS_PYTHON_CMD[@]}" scripts/generate_release_results.py
bash scripts/run_benchmarks.sh >/tmp/noetheris_benchmarks.json
if command -v lake >/dev/null 2>&1; then
  (cd formal/lean && lake build)
fi
tmp_dir="$(mktemp -d)"
"${NOETHERIS_PYTHON_CMD[@]}" -m noetheris validate-ir examples/structural_ir/consensus_safety_ir.json >/tmp/noetheris_cli_validate.json
"${NOETHERIS_PYTHON_CMD[@]}" -m noetheris compile-qubo examples/structural_ir/consensus_safety_ir.json --problem invariant --output "${tmp_dir}/compiled.json"
"${NOETHERIS_PYTHON_CMD[@]}" -m noetheris solve "${tmp_dir}/compiled.json" --solver exact --output "${tmp_dir}/solution.json"
"${NOETHERIS_PYTHON_CMD[@]}" -m noetheris run-scenario consensus >/tmp/noetheris_cli_scenario.json
"${NOETHERIS_PYTHON_CMD[@]}" -m noetheris cv-diagnostics examples/structural_ir/cv_gkp_diagnostic_ir.json >/tmp/noetheris_cli_cv.json
"${NOETHERIS_PYTHON_CMD[@]}" -m noetheris.certificates.validate examples/example_energy_certificate.json >/tmp/noetheris_cli_cert_validate.json
"${NOETHERIS_PYTHON_CMD[@]}" -m noetheris.certificates.replay examples/example_energy_certificate.json >/tmp/noetheris_cli_cert_replay.json
"${NOETHERIS_PYTHON_CMD[@]}" - <<'PY'
...
for script in invariant_annealing_search.py pq_migration_optimizer.py consensus_safety_violation.py threshold_policy_analysis.py certificate_validation.py; do
  "${NOETHERIS_PYTHON_CMD[@]}" "examples/${script}" > "${tmp_dir}/${script}.a"
  "${NOETHERIS_PYTHON_CMD[@]}" "examples/${script}" > "${tmp_dir}/${script}.b"
